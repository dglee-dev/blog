...
const projects: ProjectItem[] = [
  {
    ...
    description: (
      <p>
        ...
        상호작용의 즐거움을 더하기위해{" "}
        <code>
          <a target="_blank" rel="noreferrer" href="https://p5js.org/">
            p5.js
          </a>
        </code>
        의 WebGL 모드를 사용하여 유저의 터치에 반응해 무너지는 3D 타일을
        구현하였습니다.
      </p>
    ),
    media: {
  ...
  },
  {
    ...
    description: (
      <p>
        ...
        의 `감정해독기(Emotion Decoder)` 프로젝트에 웹 개발자로
        참여하였습니다.
        <br />
        <br />
        마치 장면이 넘어가듯 보여지는 페이지 트랜지션을 구현하기 위해
        debounce/throttle을 활용해 스크롤 이벤트의 감도를 제어하여 트랜지션
        애니메이션과 매핑하였습니다.
      </p>
    ),
    media: {
  ...
  },
  {
    ...
    description: (
      <p>
        안동선 에디터님의 아카이브 웹사이트와 포스트 작성 및 관리를 위한
        어드민을 개발하였습니다.
        <br />
        <br />
        포스트 작성 시 사용되는 텍스트 에디터에서는 텍스트를 작성하고, 이미지를
        첨부하고, 이미지 슬라이드를 만들고, 링크를 첨부할 수 있습니다. 리치
        텍스트 에디터 구현을 위해{" "}
        <code>
          <a target="_blank" rel="noreferrer" href="https://quilljs.com/">
            Quill.js
          </a>
        </code>{" "}
        를 사용하였습니다.
  ...
  },
  {
    ...
    description: (
      <p>
        레벨나인의 서비스들을 소개하는 페이지를 개발하였습니다. 스크롤에 따라
        슬라이드 컨텐츠가 앞 뒤로 페이드되는 트랜지션이 특징입니다. Intersection
  ...
  },
  {
    ...
    description: (
      <p>
        수원 광교의 교육 프로그램 '닥터 레나의 실험실' 의 참여 학생들의 직접
        가져온 물건들을 스캐닝해 만든 3D 모델들을 구경할 수 있는 웹페이지를
        개발하였습니다.
        <br />
        <br />
        3D 모델들을 렌더링하기 위해{" "}
        ...
        를 사용하였으며, 인트로 화면 배경에 존재하는 키 비주얼 요소에 모션을
        적용하기 위해{" "}
        <code>
          <a target="_blank" rel="noreferrer" href="https://gsap.com/">
            GSAP
          </a>
        </code>
        를 사용하였습니다.
  ...
  },
  {
    ...
    description: (
      <p>
        서울시의 수변갤러리 프로젝트 중 일부로서 선유도공원을 기반으로 AR
        컨텐츠를 체험할 수 있는 모바일 웹 어플리케이션 `선유동화` 를
        개발하였습니다.
        <br />
        <br />
        5개의 거점에서 지도를 기반으로 선유도 공원 곳곳에 설치된 팻말의 QR을
        이용해 각 위치에 맞는 AR 컨텐츠를 체험할 수 있습니다.
        <br />
        <br />
        현실에 꽃 기둥을 그려내는 듯한 경험을 만들어내기 위해 터치 인터랙션을
        통해 실시간으로 3D 파티클을 그릴 수 있는 기능을 프로토타이핑하여
        제안하였고 디자이너와 함께 발전시켜 완성하였습니다.
      </p>
    ),
    media: {
      type: "video",
  ...
  },
  {
    ...
    description: (
      <p>
        ...
        <br />
        <br />
        모험을 마치며 용을 부화시킬 수 있으며, 부화가 완료되었을 때 PWA의 경우
        유저에게 푸시알림을 통해 부화 완료를 알립니다.
      </p>
    ),
    media: {
      type: "video",
  ...
  },
  {
    ...
    description: (
      <p>
        ...
        <br />
        <br />
        4종의 AR 컨텐츠를 확인할 수 있으며, UI 개발에는 Web Component를 사용해
        코드 재사용성을 높였습니다. 인터랙션에 따라 재생과 정지가 가능한 Marquee
        컴포넌트가 특징입니다.
        <br />
        <br />웹 AR 라이브러리{" "}
        <code>
        ...
        의 얼굴인식 기능을 이용하여 페이스필터 컨텐츠를 개발하였으며, 좌표 위치
        기반으로 증강되는 AR 컨텐츠 개발을 위해서는{" "}
        <code>
          <a
            target="_blank"
            rel="noreferrer"
  ...
  },
  {
    ...
    description: (
      <p>
        영등포 타임스퀘어 앞 광장에 위치한 문화라운지 따따따에 전시된 설치형
        인터랙티브 미디어 3종을 개발하였습니다.
        <br />
        <br />
        조이스틱을 사용해 미디어를 조작할 수 있으며 각각 다른 종류의 정보를
        탐색하고, 게임 형식을 통해 얻어진 데이터를 기반으로 자신에게 어울리는
        미디어를 추천받을 수 있습니다.
        <br />
        <br />
  ...
  },
  {
    ...
    description: (
      <p>
        ...
        <br />
        <br />
        높은 퀄리티의 AR 컨텐츠를 제공하기 위해 iOS와 안드로이드 각각의 네이티브
        AR SDK를 이용할 수 있도록 돕는{" "}
        <code>
          <a target="_blank" rel="noreferrer" href="https://modelviewer.dev/">
            model-viewer
          </a>
        </code>
        를 사용하였습니다.
  ...
  },
  {
    ...
    description: (
      <p>
        ...
        <br />
        <br />
        비주얼 컨셉에 어울리는 마이크로 인터랙션을 위해 노이즈필터를 구현하여
        적용하였습니다.
      </p>
    ),
    media: {
  ...
  },
  {
    ...
    description: (
      <p>
        ...
        개발하였습니다. 전시장 내부에 설치된 프로젝터와 트랙패드를 이용해 Piet
        Oudolf의 정원 설계도를 기반으로 한 정원 설계도를 탐색할 수 있습니다.
        <br />
        <br />
        ...
        <br />
        <br />
        식재 정보가 화면을 벗어나지 않도록 모달 크기와 뷰포트 바운더리를
        계산하여 모달 위치를 설정하도록 하였습니다.
      </p>
    ),
    media: {
      type: "image",
  ...
  },
  {
    ...
    description: (
      <p>
        뉴욕의 사진가 사울레이터를 다룬 피크닉의 전시 `창문을 통해 어렴풋이`에
        전시된 이미지 슬라이더를 개발하였습니다.
        <br />
        <br />
  ...
  },
  {
    ...
    description: (
      <p>
        찹스틱스의 판매자 플랫폼을 개발하였습니다. 판매자로 승인된 샵의 정보를
        등록하고, 해당 샵의 상품들을 등록할 수 있습니다.
        <br />
        <br />
  ...
  },
  {
    ...
    description: (
      <p>
        바닐라코딩 포털 서비스를 기획, 디자인 및 개발하였습니다. 코스 신청서를
        작성, 결제하고 코스 컨텐츠를 확인할 수 있습니다.
        <br />
        <br />
        ...
        <br />
        <br />
        Markdown 문법을 파싱해 원하는 스타일로 렌더링하기 위해 remark/rehype를
        사용하였습니다.
      </p>
    ),
    media: {
  ...
  },
  {
    ...
    description: (
      <p>
        ...
        기능이 존재합니다. 안드로이드 플레이스토어에 배포되었으나 현재는 배포가
        중단된 상태입니다.
      </p>
    ),
    media: {
